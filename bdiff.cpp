...
#include "bdiff_instructions.inc"

    DoubleInstruction *instructions = (DoubleInstruction *)instructions_arr;

    void initSourceData(diffInfoFile* s_diffInfo, unsigned char* sourceBuffer, size_t sourceBufferSize)
    {
    ...
    void initPatchData(diffInfoFile* s_diffInfo, unsigned char* patchBuffer, size_t patchBufferSize)
    {
    ...
    unsigned char* loadSourceData(void* data, size_t offset, size_t size)
    {
        diffInfoFile* s_diffInfo = (diffInfoFile*)data;
    ...
    unsigned char* loadPatchData(void* data, size_t offset, size_t size, size_t* pOffset)
    {
        diffInfoFile* s_diffInfo = (diffInfoFile*)data;
    ...
    unsigned char* setupDestData(void* data, size_t size)
    {
        diffInfoFile* s_diffInfo = (diffInfoFile*)data;
        ...
        unsigned char* (__fastcall* setupDestData)(void* s_diffInfo, size_t size))
    {
        ...
        {
            unsigned char* header = loadPatchData(s_diffInfo->state, 0, 0x405, 0);
            if (!header)
                return false;
            ...
            loadPatchData(s_diffInfo->state, 5, 0, 0);
            s_diffInfo->headerRead = true;
        }

        unsigned __int64 diffFileStart;
        unsigned char* v13 = loadPatchData(s_diffInfo->state, 0, 0x400, &diffFileStart);
        ...
        if (v16)
        {
            ...
            __int64 v27 = v25 - v28;
            auto v32 = loadSourceData(s_diffInfo->state, v28, v22 + v27);
            v83 = &v32[v27];
        }
        __int64 v33 = readULEB128(&v13);
        auto data = loadPatchData(s_diffInfo->state, diffFileStart + v13 - headerStart, v33 + ((type >> 1) & 4), &diffFileStart);
        auto dataStart = data;

        __int64 v40 = readULEB128(&data);
        auto v44 = setupDestData(s_diffInfo->state, v40);
        ...
        loadPatchData(s_diffInfo->state, diffFileStart + v89 - dataStart, 0, 0);
    ...
    {
        diffInfo s_diffInfo;
        diffInfoFile s_diffInfoFile;
        memset(&s_diffInfoFile, 0, sizeof(s_diffInfoFile));
        memset(&s_diffInfo, 0, sizeof(s_diffInfo));

        initSourceData(&s_diffInfoFile, sourceBuffer, sourceBufferSize);
        initPatchData(&s_diffInfoFile, patchBuffer, patchBufferSize);

        s_diffInfo.state = &s_diffInfoFile;
        ...
        if (outBufferSize > 0)
        {
            do
            {
                ...
                memcpy(outBuffer, s_diffInfoFile.destBuffer, s_diffInfoFile.destBufferSize);

                outBuffer += s_diffInfoFile.destBufferSize;
                outBufferSize -= s_diffInfoFile.destBufferSize;
            } 
